...
func loadDataset(filePath string, skipHeader bool, nameRow int, coordinateStart int, coordinatesEnd int, delimiter rune) []DataPoint {
	dataset := []DataPoint{}
	...
	for {
		...
		dataset = append(dataset, DataPoint{name, &Vertex{0, coordinates}})
	}
	return dataset
}
func randomDataset(size int, dim int) []DataPoint {
	...
	for point := 0; point < size; point++ {
		...
		dataset = append(dataset, DataPoint{"", &Vertex{0, coordinates[:]}})
	}
	return dataset
}
...
func TestInsertDatasetIntoGraph(t *testing.T) {
	...
	path := "."
	graph, err := factory.New(path, "euclidean")
	assert.NoError(err)
	assert.NotNil(graph)
	var f uint16 = 4
	var w uint16 = 2
	...
	for i, j := range rand.Perm(len(dataset)) {
		...
		point := dataset[j]
		graph.NNInsert(point.vertex, f, w)
	}
	_ = graph.String()
}
...
func TestCalculateRecallAccuracyConfusionMatrix(t *testing.T) {
	...
	for i, point1 := range dataset {
		...
		for j, point2 := range dataset {
			distances = append(distances, neighbor{j, euclideanDistance(point1.vertex, point2.vertex)})
		}
		// Sort by distance
	...
	predictionsO := make(map[int][]Vertex)
	for j, point := range dataset {
		preds, _ := graph.NNSearch(point.vertex, m, k)
		var predO []Vertex
